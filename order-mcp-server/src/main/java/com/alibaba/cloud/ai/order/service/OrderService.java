...
public class OrderService {
    ...
    public Order createOrder(String productName, String sweetness, String iceLevel, int quantity) {
        ...
        try {
            ...
            if (product == null) {
                throw new IllegalArgumentException("校园服务事项不存在或当前不可用: " + productName);
            }
            
            // 检查剩余名额
            if (product.getStock() < quantity) {
                String errorMsg = String.format("剩余名额不足，服务事项: %s, 当前剩余名额: %d, 需要数量: %d", 
                        productName, product.getStock(), quantity);
                logger.error("创建办理记录失败: {}", errorMsg);
                throw new IllegalArgumentException(errorMsg);
            }
            
            BigDecimal unitPrice = product.getPrice();
            BigDecimal totalPrice = unitPrice.multiply(new BigDecimal(quantity));
            String orderId = "CAMPUS_" + System.currentTimeMillis();
    ...
    public OrderResponse createOrder(OrderCreateRequest request) {
        ...
        try {
            ...
            if (product == null) {
                throw new IllegalArgumentException("校园服务事项不存在或当前不可用: " + request.getProductName());
            }
     ...
     * 根据用户ID查询最近一次订单
     */
    public OrderResponse getLatestOrderByUserId(Long userId) {
        logger.info("=== OrderService.getLatestOrderByUserId 入口 ===");
        logger.info("请求参数 - userId: {}", userId);

        try {
            validateUser(userId);

            Order order = orderMapper.selectLatestByUserId(userId);
            if (order == null) {
                logger.info("=== OrderService.getLatestOrderByUserId 出口 ===");
                logger.info("返回结果 - 用户暂无历史订单");
                return null;
            }
    ...
    /**
     * 优先级或时间偏好字符串转数字。底层复用 ice_level 字段。
     */
    private Integer convertIceLevelToNumber(String iceLevel) {
        if (iceLevel == null) return 1;
