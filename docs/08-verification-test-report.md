...
### 6.6.5 输出膨胀优化：增量·差异方案（2026-08-05 已落地）

> 针对 6.6.4 的输出 token 膨胀（+572%），将增量方案改为**"仅输出增量差异"**：
> 增量 Prompt 只要求模型输出新片段中的【新增】关键事实（每行一条），代码中追加进已有摘要。
> Java 节点 `IncrementalContextCompressionNode.summarize()` 已同步改造。

**三方真实 LLM 对比（40 轮，qwen-plus，同一轨迹）**：

| 指标 | 滑动窗口 | 增量·合并 | **增量·差异** | 差异 vs 滑动窗口 | 差异 vs 合并 |
|---|---|---|---|---|---|
| 摘要调用次数 | 5 | 7 | 7 | +40.0% | +0.0% |
| 单次最大 prompt token | 2,254 | 1,528 | 1,558 | **-30.9%** | +2.0% |
| 累计输入 token | 7,712 | 8,600 | 8,977 | +16.4% | +4.4% |
| 累计输出 token | 403 | 1,744 | **679** | +68.5% | **-61.1%** |
| 总 token（输入+输出） | 8,115 | 10,344 | **9,656** | +19.0% | **-6.7%** |
| 最终消息条数 | 18 | 19 | 19 | +5.6% | +0.0% |
| **摘要实体保留率(%)** | 5.0 | 35.0 | **47.5** | **+850%** | **+35.7%** |

**结论**：增量·差异方案在**三项关键指标上均最优**：
- 输出 token 较合并版 **-61.1%**（彻底解决输出膨胀）；
- 摘要实体保留率 **47.5%** > 合并 35% > 滑动窗口 5%（差异追加反而保留更多实体，因为不重写摘要就不丢旧实体）；
- 单次最大 prompt 保持上界（较滑动窗口 -30.9%）。

### 6.6.6 结论解读（综合 Mock 与真实 LLM）

| 观察 | 结论 |
|---|---|
| **单次调用最大输入有上界（-19% Mock / -30.9% 真实）** | 批量上限（10 条）使摘要调用的输入规模有上界——突发大量消息时，增量方案不会出现一次超长 LLM 调用，**单次延迟可控**（核心收益） |
| **摘要实体保留率（真实 LLM）5% vs 35-80% vs 47.5%** | **最强差异化证据**：滑动窗口"一次性重摘要"在真实 LLM 下丢弃 95% 的关键记录编号；增量·差异方案保留最高（47.5%），且因不重写摘要而不丢旧实体（合并版每次重写有随机丢弃） |
| **累计输入 +6.1%（Mock）/+15.5%（真实）** | 增量方案调用更频繁且摘要逐轮增长被反复携带，总输入 token 略增 |
| **输出膨胀已解决：增量·差异较合并版 -61.1%** | 差异追加只输出新事实，累计输出 token 679（vs 合并 1,744、滑动窗口 403）；总 token 较合并版 -6.7% |
...
**适用建议（结合真实 LLM 数据）**：
- **推荐采用「增量·差异」方案**（已落地为 Java 节点默认行为）：单次输入有上界（-30.9%）、实体保留最高（47.5% vs 5%）、输出膨胀已消除（较合并版 -61%）。
- 若对**总 token 成本**极度敏感且历史实体不关键 → 滑动窗口总 token 更省（约 -16%），可作为成本优先选项。
- 两者可共存（滑动窗口 `?mode=compress`，增量·差异 `?mode=incremental-compress`），可线上 A/B。
...
### 6.7.3 运行时三维准确率（2026-08-05 新增，`scripts/golden_set_runtime.py`）

> 使用各 Agent 在 application.yml 中的**真实 Prompt + DashScope LLM**，对 30 条 GoldenCase 离线执行：
> 复用 supervisor 路由 Prompt 预测路由、子 Agent Prompt 预测工具序列、生成回复核对关键词。
> （全链路 Agent 执行受环境限制，此为 Prompt+LLM 级代理评测；工具序列比对已过滤基础设施预节点 `memory-search`。）

| 维度 | 准确率 | 说明 |
|---|---|---|
| **路由准确率** | **100.0%**（30/30） | supervisor 路由 Prompt 对所有用例判对子 Agent |
| **工具准确率** | 精确 **62.1%**（18/29） / 首工具 **79.3%**（23/29） | 29 条业务工具用例；LLM 预测序列与 expected_tools 比对（过滤 memory-search 后） |
| **关键词命中率** | **75.3%**（70/93） | 生成回复命中 expected_output_keywords 比例 |

**解读**：
- **路由维度强**（100%）——supervisor Prompt 的路由规则 + Few-shot 覆盖了全部 30 条用例。
- **工具维度中等**（62.1% 精确 / 79.3% 首工具）——部分用例 LLM 预测的工具与 golden 预期存在顺序/取舍差异（如 consult 的 `consult-search-products` vs `consult-get-product-info`），说明工具选择 Prompt 仍有优化空间；真实 Agent 执行（ReactAgent 工具调用 + 校验节点）可能更贴近预期。
- **关键词维度中等偏上**（75.3%）——受知识库覆盖（如"自习室"类查询）与回复生成方式影响。
- 该评测可作为 **H-1"三维准确率"的 Prompt 级基线**；全链路（supervisor→子 Agent→MCP 工具）执行待 Agent 运行环境。
...
| H-1 | 30 条 Golden Set 量化三维准确率 | ⚠️ **部分落地** | 结构校验已实现并通过（30 条 0 问题，见 §6.7）；**运行时三维准确率**已产出 Prompt 级基线（路由 100%/工具 62.1%/关键词 75.3%），全链路 Agent 执行仍待应用运行环境 |
